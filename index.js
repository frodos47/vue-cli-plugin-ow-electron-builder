...
module.exports = (api, options) => {
  ...
  api.registerCommand(
    'ow-electron:build',
    {
      description: 'build app with ow-electron-builder',
      usage: 'vue-cli-service ow-electron:build [ow-electron-builder options]',
      details:
        'All ow-electron-builder command line options are supported.\n' +
        'See https://www.electron.build/cli for cli options\n'
    },
    async (args, rawArgs) => {
      // Use custom config for webpack
      ...
      removeArg('--report', 1, rawArgs)
      removeArg('--report-json', 1, rawArgs)
      // Parse the raw arguments using ow-electron-builder yargs config
      const builderArgs = yargs
        .command(['build', '*'], 'Build', configureBuildCommand)
        .parse(rawArgs)
      // Base config used in ow-electron-builder build
      const outputDir = args.dest || pluginOptions.outputDir || 'dist_ow_electron'
      const defaultBuildConfig = {
        directories: {
          output: outputDir,
          app: `${outputDir}/bundled`
      ...
      // User-defined ow-electron-builder config, overwrites/adds to default config
      const userBuildConfig = pluginOptions.builderOptions || {}
      if (args.skipBundle) {
        console.log('Not bundling app as --skipBundle was passed')
        // Build with ow-electron-builder
        buildApp(args.skipElectronBuild)
      } else {
        const bundleOutputDir = path.join(outputDir, 'bundled')
        // Arguments to be passed to renderer build
        ...
        // Prevent ow-electron-builder from installing app deps
        fs.ensureDirSync(`${outputDir}/bundled/node_modules`)
      ...
      function buildApp (skip) {
        ...
        info('Building app with ow-electron-builder:')
        // Build the app using electron builder
        builder
          .build(
            merge({
  ...
  api.registerCommand(
    ...
    {
      description: 'serve app and launch electron',
      usage: 'vue-cli-service ow-electron:serve',
      details:
        'See https://nklayman.github.io/vue-cli-plugin-electron-builder/ for more details about this plugin.'
    },
    async (args, rawArgs) => {
      ...
      const outputDir = pluginOptions.outputDir || 'dist_ow_electron'
      ...
      async function launchElectron () {
        ...
        if (args.debug) {
          // Do not launch electron and provide instructions on launching through debugger
          info(
            'Not launching ow-electron as debug argument was passed. You must launch ow-electron through your debugger.'
          )
          info(
            'If you are using Spectron, make sure to set the IS_TEST env variable to true.'
          )
